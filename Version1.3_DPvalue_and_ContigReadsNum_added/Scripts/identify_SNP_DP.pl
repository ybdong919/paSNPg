...
my @inputfiles = ("All_SNP_Genotypes.txt","part_Clean_SNP_Genotypes.txt");
my @outputfiles = ("All_SNP_hap.txt","part_Clean_SNP_hap.txt");
my $xnew=0;
foreach (@inputfiles){
    chomp $_;
    my $dire= $_;
    ...
    foreach (@refe){
        my @ref_cel= split //,$_;
        $_ = $ref_cel[0];                       ### haplotype rule: select first letter
    }
